...
let part1 =
    parse >> Seq.chunkBySize 2 >> indexWithOffset 1
...
let part2 =
    parse >> Seq.append additions >> Seq.sortWith plumb >> indexWithOffset 1
    >> Seq.filter (snd >> (=) >> flip Seq.exists additions)
    >> Seq.map fst >> Seq.reduce (*)
